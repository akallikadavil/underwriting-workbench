...
async function main() {
  ...
  const rules = await Promise.all([
    ...
    prisma.rule.create({ data: { ruleName: 'Reinsurance Slip Missing Block', businessType: BusinessType.REINSURANCE_INWARD, ruleType: RuleType.BLOCK, severity: RuleSeverity.HIGH, conditionJson: { field: 'document', operator: 'missing', value: 'REINSURANCE_SLIP' }, actionJson: { action: 'BLOCK_SUBMISSION', message: 'Reinsurance Inward submissions require a reinsurance slip.' }, description: 'Reinsurance slip mandatory for inward reinsurance', active: true } }),
  ...
  for (const sub of submissionData) {
    ...
    const created = await prisma.submission.create({
      data: {
        ...
        assignedToUserId: [SubmissionStatus.UNDERWRITING_REVIEW, SubmissionStatus.REFERRAL_REQUIRED, SubmissionStatus.SENIOR_REVIEW, SubmissionStatus.ASSIGNED_TO_UNDERWRITING].includes(sub.status as SubmissionStatus) ? underwriter.id : undefined,
